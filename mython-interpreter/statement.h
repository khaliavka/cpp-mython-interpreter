...
public:
    ...
    runtime::ObjectHolder Execute(runtime::Closure& closure, runtime::Context& context) override;
private:
    const runtime::Class& class_; //Check
    std::vector<std::unique_ptr<Statement>> args_;
};
...
public:
    explicit UnaryOperation(std::unique_ptr<Statement> arg) : argument_{std::move(arg)} {
    }
protected:
    const std::unique_ptr<Statement>& GetArgument() const;
private:
...
protected:
    const std::unique_ptr<Statement>& GetLeftArgument() const;
    const std::unique_ptr<Statement>& GetRightArgument() const;
private:
...
public:
    ...
    runtime::ObjectHolder Execute(runtime::Closure& closure, runtime::Context& context) override;
private:
    runtime::ObjectHolder class_definition_;
};
