...
void TestNumberSubtraction() {
    ...
    Sub sub(make_unique<NumericConst>(23), make_unique<NumericConst>(34));

    Closure empty;
    ASSERT_OBJECT_VALUE_EQUAL(sub.Execute(empty, context), -11);
...
void TestCompound() {
    ...
    cpd.AddStatement(make_unique<Assignment>("BladeRunner"s, make_unique<NumericConst>(2049)));
    Closure closure;
    ...
    ASSERT_OBJECT_VALUE_EQUAL(closure.at("BladeRunner"s), 2049);
...
void RunUnitTests(TestRunner& tr) {
    ...
    RUN_TEST(tr, ast::TestNumberSubtraction);
